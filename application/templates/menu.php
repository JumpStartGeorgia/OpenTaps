...
foreach (Storage::instance()->menu AS $menu):
    ...
    <li<?php $has AND print ' class="dropdownmenu"' ?> id="<?php echo $menu['unique'] ?>">
        <a class="menu-item" href="<?php echo href('page/' . $menu['short_name'], TRUE) ?>"><div style="float: left"><?php echo LANG == 'ka' ? geo_utf8_to_latin($menu['name']) : strtoupper(strtolower($menu['name'])) ?></div><?php echo $has ? $span : NULL ?></a>
    </li>
<?php endforeach; ?>
