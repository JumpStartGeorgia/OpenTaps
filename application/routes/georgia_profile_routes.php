...
Slim::post('/admin/georgia_profile/', function()
        {
            ...
            if (!empty($_POST['content']) AND strlen($_POST['content']) > 1)
            {
                fetch_db(
		    	 ...
		    	 INSERT INTO georgia_profile_content(`lang`, `value`) VALUES('" . LANG . "', :value)", array(':value' => $_POST['content']));
            }
            if (!empty($_FILES['image']) AND $_FILES['image']['size'] > 0)
            {
                ...
                fetch_db(
		    	 ...
		    	 INSERT INTO georgia_profile(`unique`, `key`, `lang`, `value`) VALUES(:unique, 'datatype_image', '" . LANG . "', :value)", array(':unique' => generate_unique('georgia_profile'), ':value' => image_upload($_FILES['image']))
                );
            }
